...
export async function POST(request: NextRequest) {
  try {
    ...
    const keyPath = path.join(pkiPaths.clientKeysPath, `${certName}.key`)
    const certPath = path.join(pkiPaths.clientCertsPath, `${certName}.pem`)
    const pfxPath = path.join(pkiPaths.clientPkcs12Path, `${certName}.p12`)
