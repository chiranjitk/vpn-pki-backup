  ...
  const fetchOcspConfig = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        if (data.config) {
          setOcspConfig(data.config)
        }
      }
    } catch (error) {
      console.error('Failed to fetch OCSP config:', error)
    }
  }
  ...
  const handleSaveOcsp = async () => {
    ...
    try {
      const response = await fetch('/api/ocsp', {
        ...
        body: JSON.stringify(ocspConfig),
      })
      if (response.ok) {
        toast.success('OCSP configuration saved')
        fetchOcspConfig()
      } else {
        throw new Error('Failed to save')
      }
    } catch (error) {
      toast.error('Failed to save OCSP configuration')
    } finally {
      setSavingOcsp(false)
    }
  }
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="ocsp-url">Responder URL</Label>
                <Input
                  id="ocsp-url"
                  placeholder="http://localhost:3033"
                  value={ocspConfig.responderUrl}
                  onChange={(e) => setOcspConfig({ ...ocspConfig, responderUrl: e.target.value })}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="ocsp-port">Port</Label>
                <Input
                  id="ocsp-port"
                  type="number"
                  value={ocspConfig.port}
                  onChange={(e) => setOcspConfig({ ...ocspConfig, port: parseInt(e.target.value) || 3033 })}
                />
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="ocsp-cert">OCSP Certificate Path</Label>
                <Input
                  id="ocsp-cert"
                  placeholder="/etc/swanctl/ocsp.crt"
                  value={ocspConfig.ocspCertPath}
                  onChange={(e) => setOcspConfig({ ...ocspConfig, ocspCertPath: e.target.value })}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="ocsp-key">OCSP Key Path</Label>
                <Input
                  id="ocsp-key"
                  placeholder="/etc/swanctl/ocsp.key"
                  value={ocspConfig.ocspKeyPath}
                  onChange={(e) => setOcspConfig({ ...ocspConfig, ocspKeyPath: e.target.value })}
                />
              </div>
            </div>

            <div className="space-y-2">
              <Label htmlFor="ca-cert">CA Certificate Path</Label>
              <Input
                id="ca-cert"
                placeholder="/etc/swanctl/x509ca/intermediate.crt"
                value={ocspConfig.caCertPath}
                onChange={(e) => setOcspConfig({ ...ocspConfig, caCertPath: e.target.value })}
              />
            </div>

            <div className="flex gap-2">
              <Button onClick={handleSaveOcsp} disabled={savingOcsp}>
                {savingOcsp ? (
                  <>
                    <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                    Saving...
              ...
              <Button variant="outline" onClick={fetchOcspStatus}>
